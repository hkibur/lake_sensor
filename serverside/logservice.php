...
$dev_name = bin2hex(substr($data_raw, $mac_addr_len, $mac_addr_len + $device_name_len));
...
if(($data_len - $data_offset) % $total_len == 0){
    ...
    for($i = $data_offset; $i < $data_len; $i += $total_len){
        ...
        $log_string = date("Y-m-d H:i:s", intval($average_time / 1000)) . ":" . strval($average_time % 10000) . " | " . strval($unpacked_arr["average"]) . " | " . bin2hex($unpacked_arr["error"]) . "\n";
    ...
    echo hex2bin(fread($ret_handle, $config_len));
    fclose($ret_handle);
}
?>